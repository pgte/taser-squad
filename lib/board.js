...
function load(cb) {
  async.parallel([
    ...
    this.tiles.load.bind(this.tiles)
    ], cb);
}
...
function update(object) {
  ...
  if (oldImage != newImage.src) {
    item.element.attr('href', newImage.src);
    item.element.attr('width', newImage.width);
    item.element.attr('height', newImage.height);
  }
}
